...
class TextToSQLDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        if len(ann["input"]) < 1800:
            try:
                schema, question = ann["input"].split("\n-- -- ", 1)
            except ValueError:
                print("\n⚠️ Failed to split input at index:", index)
                print("Input string:", ann["input"])
                print("Raw bytes of input:", list(ann["input"].encode()))
                print("Index of '-- --':", ann["input"].find(" -- --"))
                raise
